...
[[nodiscard]] std::size_t checked_product(std::uint32_t left,
    ...
    if (static_cast<std::size_t>(left) > max / static_cast<std::size_t>(right)) {
...
[[nodiscard]] bool is_inside_circle(std::uint32_t x,
    ...
    // Subtract before comparing the second axis: for UINT32_MAX radius,
    // dx^2 and dy^2 each fit uint64, but their sum does not necessarily fit.
...
[[nodiscard]] ClippedBounds clipped_circle_bounds(std::uint32_t width,
    ...
    // Saturating comparisons avoid overflow for centers near int64 limits.
...
std::vector<CommandResult> World::submit_command_batch(std::span<const Command> commands) {
    // Validate the entire batch before mutation so malformed transport data
    // cannot leave an ambiguously half-applied batch.
    ...
    for (const auto& command : commands) {
        if (const auto* paint = std::get_if<PaintCircleCommand>(&command)) {
            ...
            results.emplace_back(extract_circle(std::get<ExtractCircleCommand>(command)));
        }
    }
    return results;
}

std::vector<DirtyChunkSnapshot> World::consume_dirty_chunks() {
    std::vector<DirtyChunkSnapshot> snapshots;
    snapshots.reserve(dirty_chunks_.size());
    ...
    // Building snapshots allocates. Clear flags only after every snapshot has
    // been built successfully so an allocation failure cannot partially
    // consume the pending transport batch.
    std::fill(dirty_chunks_.begin(), dirty_chunks_.end(), false);
    return snapshots;
}

WorldSnapshot World::snapshot() const {
    return {config_, tick_, random_state_, cells_};
}
