...
`native/sim_core` 已建立 Godot 无关的连续材质存储与 64 × 64 分区传输基础：表现层可以批量提交绘制/采掘命令，并按稳定顺序消费紧凑的脏区块快照。材质 ID `0..9` 与当前 GDScript 原型一致，种子、固定步编号、快照与校验和进入可验证状态。

这一基础尚未接入 GDExtension，也未迁移砂、水、火、温度和反应规则。当前可玩 Demo 中的 `MaterialWorld` GDScript 仍是权威模拟；C++ 世界只用于独立验证存储、确定性与批传输协议。接入桥接层时必须整批提交命令、整批取回脏区块，不能退化为逐像素跨边界调用。
